      ...
      tabItem(
        ...
        fluidRow(
          column(width = 12,
                 ...
                 p("Ruybal-Pesántez S and Verity R (2023). DRpower and pfhrp2/3 planner app: Study design and analysis for pfhrp2/3 deletion prevalence studies. R package version 1.0.2 and R Shiny app version 1.0.1. https://shiny.dide.ic.ac.uk/DRpower-app"),
                 br(), 
        ...
        fluidRow(
          column(width = 12, style='padding:20px;', 
                 Callout(
                   ...
                   br(), br(),
                   strong("5. Why do I need more than 95% probability to conclude that prevalence is above the 5% threshold? Does this mean I will have 95% power?"),
                   br(), br(),
                   "This question is a bit tricky and needs a detailed explanation.", 
                   br(), br(),
                   "Unlike traditional null hypothesis testing, Bayesian methods don’t usually aim to make a clear-cut decision between hypotheses. Instead, they calculate the probability of each hypothesis being true based on the data. To turn this into a decision, we need to set a cutoff — a probability level at which we decide one hypothesis is more likely than the other. For example, we could decide that prevalence is above the 5% threshold if there is more than a 50% chance that this is true. ",
                   "However, the cutoff we choose affects the chances of making errors. A 50% cutoff would give us more power (the ability to detect true positives) but would also lead to a high false-positive rate. By default, we use a 95% cutoff to reduce the risk of false positives.However, the cutoff we choose affects the chances of making errors. A 50% cutoff would give us more power (the ability to detect true positives) but would also lead to a high false-positive rate. By default, we use a 95% cutoff to reduce the risk of false positives.",
                   br(), br(),
                   "Why 95%? Before collecting any data, our conclusions depend on our starting assumptions, or ", em("prior probabilities. "),
                   "By default, we use a uniform prior, which assumes all prevalence levels are equally likely. This means 95% of the prior probability lies above the 5% threshold, so initially, it’s more likely that prevalence is high. By using a 95% cutoff, we counterbalance this assumption. This makes our starting point neutral: before seeing any data, we are equally likely to conclude that prevalence is above or below the threshold. As new data come in, they shift the evidence and help us reach a definitive conclusion.",
                   br(), br(),
                   ...
                   br(), br(),
                   strong("6. What about multiplicity of infection? Is this accounted for in this analysis?"),
                   br(), br(),
                   "Multiplicity of infection (MOI) refers to a single individual being infected with multiple parasite strains. This is particularly important in ", em("pfhrp2/3"), " studies because only one strain needs to carry the ", em("pfhrp2/3"), " gene for an HRP2-based RDT to likely return a positive result.",
                   br(), br(),
                   "Therefore, it is essential to distinguish between two key concepts:",
                   br(), 
                    "• Prevalence of gene deletions: The proportion of individuals who carry only ", em("pfhrp2/3"), "-deleted strains.",
                    br(),
                    "• Frequency of gene deletions: This can be defined in various ways, but one common definition is the probability that an infectious mosquito bite introduces a ", em("pfhrp2/3"), "-deleted strain.",
                   br(), br(),
                   ...
                   br(), br(),
                   "In the DRpower analysis, we estimate the ", em("prevalence"), " of gene deletions, not the frequency. The advantage of this approach is that prevalence is the most clinically relevant measure—it reflects the proportion of clinical cases that may be missed due to deletions. However, a limitation is that prevalence can vary with changes in transmission intensity. For instance, as malaria transmission declines, infections may become more likely to be mono-clonal, which could increase the observed prevalence of ", em("pfhrp2/3"), " deletions even if the underlying frequency remains unchanged.",
                   br(), br(),
                   ...
                   br(),
                   "1. If a polyclonal sample containing only ", em("pfhrp2/3"), "-deleted strains is classified as a deletion, and a polyclonal sample with at least one ", em("pfhrp2/3"), "-present strain is not classified as a deletion, then MOI is effectively accounted for, and no further adjustments are needed.",
                   br(),
                   ...
                   br(), br(),
                   strong("7. What do my sample size numbers actually represent?"),
                   br(), br(),
                   ...
                   br(), br(),
                   strong("8. How do I analyse my data if I have both ", em("hrp2 "), "and ", em("hrp3 "), "gene deletion counts?"),
                   br(), br(),
                   ...
                   br(), br(), br(),
                   strong("9. What if I want to perform even more bespoke and/or advanced analyses?"),
                   br(), br(),
