        ...
        fluidRow(
          column(width = 12, 
                 ...
                 div(class = "custom-callout",
                     ...
                     "1.	In the ", shinyLink(to = "design", label = "design phase"), "(before data have been collected) to help guide the appropriate number of clusters and a sample size per cluster.",
                     br(),
                     "2.	In the ", shinyLink(to = "analysis", label = "analysis phase"), "(once data are available) to estimate prevalence of deletions and determine if it is above a set threshold.",
                     br(), br(),
                     "The ideal plan would be to perform both steps, i.e., using this app before a study has started to choose target sample sizes and then returning to the app once data are available. However, it is valid to analyse data even if sample sizes were chosen using a different method (see ",shinyLink(to = "faq", label = "FAQs"), ").",
                     br(), br(),
                     "For those wanting more background information on the method, or who want to perform more advanced analyses, please take a look at the ",
            ...
            tabsetPanel(type = "tabs",
                        tabPanel("Sample size tables",
                                 ...
                                 div(class = "custom-callout",
                                    ...
                                    "The table below gives the number of confirmed malaria positive samples required ", em("per cluster "), "for a study to be powered at 80% or higher. You can use these numbers as a general guide when scoping out a study plan before moving to more tailored sample sizes in the next step.",
                                    br(), br(),
                        ...
                        tabPanel(title = "Generate report",
                                 ...
                                 box(width = 12,
                                     ...
                                     p("Click the button below to display a summary of the information you entered in the previous tab. If everything looks as expected, click on the download button to download your report."),
                                     br(),
        ...
        fluidRow(
          column(width = 12, style='padding:20px;', 
                 Callout(
                   ...
                   br(), br(),
                   "Imagine that the true prevalence in our province is 6%. It would only take a small amount of bad luck for the sample prevalence to be less than 5%, meaning we could come to the wrong conclusion that prevalence was below the 5% threshold. On the other hand, if the prevalence in our province is 20% then we would have to be extremely unlucky for the sample prevalence to dip this low. This means that our chance of coming to the correct conclusion (our power) is highest when the true prevalence is a long way from the threshold. For this reason, we cannot perform power analysis without first fixing how strong our effect size is.",
                   br(), br(),
                   strong("3. How should I decide what “true prevalence” value to assume?"),
                   br(), br(),
                   "This is a tricky question to answer, as it depends on the details of your study and your specific objectives. We can ask instead; what prevalence level do you really care about detecting, i.e., what is relevant for control purposes? Would you want to know if the prevalence of pfhrp2/3 deletions was 5.1% so that you could immediately switch RDTs? What if the prevalence was 5.001%?",
                   br(), br(),
                   "In reality, we should remember that the 5% level was chosen based on an argument that this is ",
                   em("roughly "),
                   "the level at which missed cases due to deletions match missed cases due to loss of sensitivity in alternative RDTs. We should treat this number as a useful guide, not a value for us to slavishly follow. We should also keep in mind that the closer our assumed prevalence is to the 5% threshold, the larger our sample size will need to be, up to values that are completely unrealistic for any control programme. There is a balance to be struck between sensitivity to detect a given effect size, and pragmatic arguments based on logistics, budget, and ethical considerations. Here, we opt for an assumed 10% prevalence as the default, as this gives a reasonable level of sensitivity while also leading to realistic sample sizes.",
                   br(), br(),
                   em("For more information on the statical methods used in the back-end of this app, or if you want to perform more advanced analyses, please visit the "),
