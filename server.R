...
function(input, output, session) {
  ...
  output$est_prev_resulttext <- renderUI({
    ...
    if(!is.null(prev_output()) && prev_output()$prob_above_threshold >= 0.95){
      ...
      line2 <- paste("We conclude that the ", em("pfhrp2/3"), "deletion prevalence is above the 5% threshold (", round(as.numeric(prev_output()$prob_above_threshold)*100, 2), "% probability).")
      ...
      line1 <- paste("RESULT: We estimate that the prevalence of", em("pfhrp2/3"), "deletions is ", round(as.numeric(prev_output()$MAP), 2), "% (95% CrI: ", round(as.numeric(prev_output()$CrI_lower), 2), "- ", round(as.numeric(prev_output()$CrI_upper), 2), "%).")
      line2 <- paste("We conclude that the ", em("pfhrp2/3"), "deletion prevalence is below the 5% threshold (", round(as.numeric(prev_output()$prob_above_threshold)*100, 2), "% probability).")
